...
export default async function globalSetup() {
  ...
  const { data: org, error: orgError } = await admin
    ...
    .insert({
      ...
      // processed a club's paperwork, just that an approver did.
      sofo_approvers: [E2E_EMAIL],
  ...
  // Find-or-create the test user, so repeated local runs (without a full
  // `supabase db reset` in between) don't fail on "already registered".
  ...
  if (!userId) {
    throw new Error(
      `Failed to find or create the e2e test user: ${createError?.message}`,
    );
  }
  ...
  if (linkError || !linkData) {
    throw new Error(`Failed to generate a magic link: ${linkError?.message}`);
  }
