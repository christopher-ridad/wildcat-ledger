-- Adds an explicit "member being reimbursed" name field, separate from the
-- generic transaction title (which is the expense description). Previously
-- the title field was being used ambiguously for both.

alter table transactions add column if not exists reimbursed_member_name text;
...
begin
  ...
  insert into transactions (
    ...
    zelle_info, reimbursed_member_name, is_individual_vendor, no_receipt_acknowledged,
    receipt_file_url, contract_file_url, w9_file_url, contracted_services_file_url,
