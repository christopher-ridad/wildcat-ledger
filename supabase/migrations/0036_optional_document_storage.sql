-- Lets an org choose, per document, not to keep a copy of an RSO
-- Agreement, W-9, Contracted Services Form, Conflict of Interest Form, or
-- Special Pay Form in WildcatLedger, since several of them carry personal
-- information (SSNs, addresses). The org still hands the document to SOFO
-- as usual; *_not_stored just records that it's been dealt with outside
-- the app, so it counts as present for the missing-document flags and the
-- Approved/Paid gate. Receipts have no such option: reconciliation needs
-- them. See docs/BUSINESS_RULES.md#documents-kept-outside-wildcatledger.
--
-- The *_acknowledged_missing columns (the old "I don't have this yet"
-- checkboxes) are no longer written by the app: a document that's neither
-- uploaded nor marked not stored is simply missing. They're left in place
-- for historical rows and audit entries.

alter table transactions
  add column contract_not_stored boolean not null default false,
  add column w9_not_stored boolean not null default false,
  add column contracted_services_not_stored boolean not null default false,
  add column conflict_of_interest_not_stored boolean not null default false,
  add column special_pay_form_not_stored boolean not null default false;

-- Body copied from 0035_existing_vendor.sql's definition (the current
-- one), plus the five new columns.
...
-- Body copied from 0035_existing_vendor.sql's definition (the current
-- one); a document marked not stored counts as present.
