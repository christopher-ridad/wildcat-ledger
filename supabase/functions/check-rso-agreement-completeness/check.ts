// The actual RSO Agreement completeness logic, split out from index.ts so
// it can be unit tested (see check.test.ts) against synthetic fixtures
// instead of only ever being exercised by a real, billed Document AI call.
//
...
export const SECTION_BOXES: Record<number, { page: number; box: Box }> = {
...
export const SECTION_4_ROWS: { key: string; label: string; yes: Box; no: Box }[] = [
  {
    key: 'a',
...
export function circleBoxFromToken(token: Box): Box {
...
export const RESERVATION_LABEL_TEXT =
...
export function checkRsoAgreement(
  documentText: string,
